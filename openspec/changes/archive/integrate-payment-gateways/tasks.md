...
```
frontend/src/
├── components/Payment/          # 6个组件文件
│   ├── PaymentPage.tsx          # 主支付页面
│   ├── PaymentSummary.tsx       # 订单摘要
│   ├── PaymentMethods.tsx       # 支付方式选择
│   ├── PaymentRedirect.tsx      # 支付跳转
│   ├── PaymentStatus.tsx        # 状态展示
│   └── PaymentErrorBoundary.tsx # 错误边界
├── hooks/                       # 3个自定义Hook
│   ├── useOrderInfo.ts          # 订单信息获取
│   ├── usePaymentGateway.ts     # 支付网关集成
│   └── usePaymentStatus.ts      # 支付状态轮询
├── utils/                       # 3个工具文件
│   ├── payment-api.ts           # 支付API调用
│   ├── poller.ts                # 轮询工具
│   └── validation.ts            # 输入验证（已扩展）
└── types/                       # 1个类型文件
    └── payment.ts               # 支付相关类型定义
```
...
### 🎯 **核心功能实现**
1. **双支付网关支持**: 支付宝 + Creem
2. **实时状态监控**: 3秒轮询，15分钟超时
3. **智能设备适配**: 桌面端新窗口，移动端直接跳转
4. **完整错误处理**: 网络异常、支付失败、超时处理
5. **安全验证机制**: 订单ID验证、支付参数检查、XSS防护

### 🔗 **API集成**
- `GET /api/v1/orders/{orderId}` - 订单信息获取
- `POST /api/v1/payment/init/{orderId}` - 支付初始化
- `GET /api/v1/payment/status/{orderId}` - 状态查询
- `POST /api/v1/payment/retry/{orderId}` - 支付重试

### 📱 **路由配置**
- `/payment/:orderId` - 支付页面（路由参数）
- `/payment?orderId=xxx` - 支付页面（查询参数）
