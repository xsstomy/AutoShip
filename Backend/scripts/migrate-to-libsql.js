...
async function main() {
  ...
  try {
    ...
    const indexResult = await client.execute(`
      ...
      ORDER BY name
    `)
    log(`✅ 找到 ${indexResult.rows.length} 个索引`, 'green')

    // 步骤 6: 测试 CRUD 操作
    log('\n🧪 步骤 6: 测试 CRUD 操作', 'blue')

    // 测试 SELECT
    try {
      const selectResult = await client.execute('SELECT COUNT(*) as count FROM products')
      log(`  ✅ SELECT 操作成功 - products 数量: ${selectResult.rows[0].count}`, 'green')
    } catch (error) {
      log(`  ⚠️  SELECT 操作失败 (可能是因为 products 表为空): ${error.message}`, 'yellow')
    }
    ...
    // 步骤 8: 测试 PRAGMA
    log('\n⚙️  步骤 8: 测试 PRAGMA 设置', 'blue')
    try {
      const pragmaResult = await client.execute('PRAGMA journal_mode')
      log(`  ✅ journal_mode: ${pragmaResult.rows[0].journal_mode}`, 'green')
    } catch (error) {
      log(`  ⚠️  PRAGMA 查询失败 (某些 PRAGMA 在远程模式下可能不支持): ${error.message}`, 'yellow')
    }
